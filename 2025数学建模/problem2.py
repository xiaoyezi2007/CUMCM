...
for V_FY1 in np.arange(70, 140 + V_INTERVAL, V_INTERVAL):
    print(f"\n{'=' * 20} Testing V_FY1 = {V_FY1:.1f} {'=' * 20}")
    T_max = 6

    for T in np.arange(0.0, T_max, T_INTERVAL):
        # --- 1. 定义当前T时刻的几何体 (无需改变) ---
        ...
        # # --- 旧的、有问题的剪枝逻辑 ---
        # dist_m1_to_cir_center = np.linalg.norm(pos_m1_T[:2] - cir_center_xy)
        # if dist_m1_to_cir_center > cir_radius + 10.0:
        #     continue
        ...
        # 圆心到整个三角形的最短距离，是到三条边距离的最小值
        min_dist_to_triangle = min(dist_to_edge1, dist_to_edge2, dist_to_edge3)

        # 只有当三角形离圆心的最短距离也大于圆的半径时，它们才不可能相交
        if min_dist_to_triangle > cir_radius:
            # 判断圆心是否在三角形内部，如果在内部，则距离为0，不会被剪枝
            # (point_segment_distance已隐式处理，因为如果圆心在内，到边的距离会很小)
            continue  # 安全剪枝

        # --- 3. 计算几何相交区域 (无需改变) ---
        triangle = Polygon([v1, v2, v3])
        circle_boundary = LineString([
            (cir_center_xy[0] + cir_radius * np.cos(a), cir_center_xy[1] + cir_radius * np.sin(a))
        ...
        for arc in candidate_arcs:
            ...
            for angle in np.linspace(start_angle, end_angle, num_angle_steps):
                g_xy = np.array([
                    cir_center_xy[0] + cir_radius * np.cos(angle),
                    cir_center_xy[1] + cir_radius * np.sin(angle)
                ])
                num_z_steps = int(abs(z_max - z_min) / Z_STEP) + 1 if Z_STEP > 0 else 1
                for z in np.linspace(z_min, z_max, num_z_steps):
...
if global_max_ans > -1:
    # --- 1. 提取和计算最终参数 ---
    # (这部分计算不变)
    V_FY1_opt = global_best_params['V_FY1']
    T_opt = global_best_params['T']
    P_G_opt = global_best_params['P_G']

    displacement_fy1_xy = P_G_opt[:2] - P_FY1_start[:2]
    norm = np.linalg.norm(displacement_fy1_xy)
    if norm > 0:
        dir_fy1_opt = displacement_fy1_xy / norm
    else:
        dir_fy1_opt = np.array([0.0, 0.0])

    delta_z = P_G_opt[2] - P_FY1_start[2]
    t2_opt = np.sqrt(2 * delta_z / A_G_Z)
    t1_opt = T_opt - t2_opt
